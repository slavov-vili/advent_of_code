...
public class Grid2DInfinite<T> implements IGrid2D<T> {
	...
	public Set<Point> getPositionsWhere(Predicate<T> condition) {
		return this.grid.keySet().stream().filter(position -> condition.test(this.get(position)))
	...
	public long count(Predicate<T> condition) {
		return this.getPositionsWhere(condition).size();
	}
	...
	private int getMax(Function<Point, Double> coordGetter) {
		return this.grid.keySet().stream().mapToInt(curPoint -> coordGetter.apply(curPoint).intValue()).max()
				.getAsInt();
	}

	private int getMin(Function<Point, Double> coordGetter) {
		return this.grid.keySet().stream().mapToInt(curPoint -> coordGetter.apply(curPoint).intValue()).min()
				.getAsInt();
	}
